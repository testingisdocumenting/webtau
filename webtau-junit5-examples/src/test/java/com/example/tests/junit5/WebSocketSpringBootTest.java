...
public class WebSocketSpringBootTest {
    ...
    public void waitUntilReceiveMessage() {
        WebSocketSession wsSession = websocket.connect("ws://localhost:" + DynamicPortBaseUrlConfig.getSpringbootAppPort() + "/prices");
        wsSession.sendText("{\"message\": \"hi\"}");

        wsSession.received.waitTo(equal(map("price", 100)));
    }
}
