...
public class TableDataJavaTestValidations {
    ...
    public static void validatePermute(TableData tableData) {
        ...
        actual(tableData.row(0).toMap()).should(equal(
                map("Col A", true, "Col B", "v1b", "Col C", "a")));
        actual(tableData.row(1).toMap()).should(equal(
                map("Col A", false, "Col B", "v1b", "Col C", "a")));
        actual(tableData.row(2).toMap()).should(equal(
                map("Col A", true, "Col B", "v1b", "Col C", "b")));
        actual(tableData.row(3).toMap()).should(equal(
                map("Col A", false, "Col B", "v1b", "Col C", "b")));
        actual(tableData.row(4).toMap()).should(equal(
                map("Col A", "v2a", "Col B", 10, "Col C", "v2c")));
        actual(tableData.row(5).toMap()).should(equal(
                map("Col A", "v2a", "Col B", 20, "Col C", "v2c")));
    }

    public static void validatePermuteAndGuid(TableData tableData) {
        actual(tableData.numberOfRows()).should(equal(6));
        actual(tableData.row(0).toMap()).should(equal(
                map("ID", notEqual(""), "Col A", true, "Col B", "v1b", "Col C", "a")));
        actual(tableData.row(1).toMap()).should(equal(
                map("ID", notEqual(""), "Col A", false, "Col B", "v1b", "Col C", "a")));
        actual(tableData.row(2).toMap()).should(equal(
                map("ID", notEqual(""), "Col A", true, "Col B", "v1b", "Col C", "b")));
        actual(tableData.row(3).toMap()).should(equal(
                map("ID", notEqual(""), "Col A", false, "Col B", "v1b", "Col C", "b")));
        actual(tableData.row(4).toMap()).should(equal(
                map("ID", notEqual(""), "Col A", "v2a", "Col B", 10, "Col C", "v2c")));
        actual(tableData.row(5).toMap()).should(equal(
                map("ID", notEqual(""), "Col A", "v2a", "Col B", 20, "Col C", "v2c")));
    ...
    public static void validateAboveValue(TableData tableData) {
        ...
        actual(tableData.row(0).toMap()).should(equal(
                map("Name", "John", "Start Date", firstDate, "Games To Play", 10)));
        actual(tableData.row(1).toMap()).should(equal(
                map("Name", "Bob", "Start Date", firstDate, "Games To Play", 8)));
        actual(tableData.row(2).toMap()).should(equal(
                map("Name", "Mike", "Start Date", firstDate, "Games To Play", 14)));

        LocalDate secondDate = LocalDate.of(2016, 6, 22);
        actual(tableData.row(3).toMap()).should(equal(
                map("Name", "Drew", "Start Date", secondDate, "Games To Play", 10)));
        actual(tableData.row(4).toMap()).should(equal(
                map("Name", "Pete", "Start Date", secondDate, "Games To Play", 11)));
        actual(tableData.row(5).toMap()).should(equal(
                map("Name", "Max", "Start Date", secondDate, "Games To Play", 3)));
    }
    ...
    public static void validateAboveValueWithMath(TableData tableData) {
        ...
        actual(tableData.row(0).toMap()).should(equal(
                map("Name", "John", "Start Date", firstDate, "Games To Play", 10)));
        actual(tableData.row(1).toMap()).should(equal(
                map("Name", "Bob", "Start Date", firstDate, "Games To Play", 11)));
        actual(tableData.row(2).toMap()).should(equal(
                map("Name", "Mike", "Start Date", firstDate, "Games To Play", 12)));
    }
}
